...
function Board({ nft, buyers }: BoardProps) {
    ...
    useEffect(() => {
        const loadNFTInfo = async () => {
            ...
            // Get all listed NFTs
            const mPContract = new web3.eth.Contract(Marketplace.abi as any, Marketplace.networks[`${networkId}` as keyof typeof Marketplace.networks]['address'])
            setMarketPlaceContract(mPContract)
            const eNFTContract = new web3.eth.Contract(EncodedNft.abi as any, EncodedNft.networks[`${networkId}` as keyof typeof EncodedNft.networks]['address'])
            setEncodedNftContract(eNFTContract)
        }
        loadNFTInfo()
    }, [nft, buyers])
